...
fn main() {
    let gw_path = std::env::args().nth(1).expect("Usage: bench_real <gw.fits> <grb.fits> [n_trials]");
    let grb_path = std::env::args().nth(2).expect("Usage: bench_real <gw.fits> <grb.fits> [n_trials]");
    let n_trials: usize = std::env::args().nth(3).map(|s| s.parse().unwrap()).unwrap_or(1000);

    println!("Loading GW skymap: {}", gw_path);
    let t0 = Instant::now();
    let gw = SparseSkymap::from_fits(&gw_path).unwrap();
    println!("  NSIDE={}, nnz={}/{} ({:.1}%), loaded in {:.1}ms",
        gw.nside, gw.nnz(), gw.npix(),
    ...
    println!("\nOverlap integral: {:.6e} (computed in {:.3}ms)",
        ov, t0.elapsed().as_secs_f64() * 1000.0);
    ...
    println!("Time per trial:   {:.2}ms", elapsed * 1000.0 / n_trials as f64);
    println!("Throughput:       {:.0} trials/sec", n_trials as f64 / elapsed);
    ...
    println!("\nJoint FAR (Piotrzkowski): {:.4e} Hz ({:.2e} /year)",
        far, far * 365.25 * 86400.0);
}
